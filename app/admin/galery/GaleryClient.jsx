...
const GaleryClient = (props) => {
  const data = props?.galery;
  ...
  const onDelete = async (data) => {
    Swal.fire({
      title: data.id + " Indexli Resim Silinecektir! ",
      showDenyButton: true,
  ...
  const addGalery = async () => {
    if (cimage.length < 0) {
      ...
      const res = await setGalery(data);
      if (res === true) {
        await Swal.fire({
          icon: "success",
        ...
        <div className="grid grid-cols-4 py-6 space-x-12 rounded-2xl bg-white mb-10">
          ...
          <div className="flex-1 flex items-center justify-center ">
            <div className="ml-12 text-red-600 font-semibold text-lg text-center">
              Resimleri Yükledikten Sonra Lütfen Kaydete Basınız...
            </div>
          </div>
          <div className="flex-1 flex items-center justify-center">
